...
export class GcpLogger {
  ...
  constructor(options: GcpLoggerOptions = {}) {
    this.opts = {
      redactHeaders: (options.redactHeaders ?? ["authorization", "cookie"]).map(h => h.toLowerCase()),
      captureBody: options.captureBody ?? false,
      maxBodyLength: options.maxBodyLength ?? 10_000,
      getUserId: options.getUserId ?? (() => undefined),
      skipPaths: options.skipPaths ?? [/^\/healthz$/, /^\/metrics$/, /^\/favicon\.ico$/],
  ...
  private safeHeaders(req: Request): Record<string, string | string[] | undefined> {
    const headers: Record<string, any> = {};
    for (const [k, v] of Object.entries(req.headers)) {
      headers[k] = this.opts.redactHeaders.includes(k.toLowerCase()) ? "[REDACTED]" : v;
    }
    return headers;
  }
